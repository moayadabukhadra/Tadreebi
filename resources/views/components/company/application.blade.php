<x-layout>
    ...
    <div class="mt-8 overflow-hidden bg-white shadow sm:rounded-lg">
        <div class="flex gap-40 px-4 py-5 sm:px-6">
            <div>
            ...
            <a href="/student/dashboard/{{$application->student->id}}" class="p-3 text-gray-700 bg-blue-400 rounded-xl"> View Profile</a>
        </div>
        ...
        <div class="border-t border-gray-200">
            <dl>
                <div class="px-4 py-5 bg-gray-50 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $application->student->name }}</dd>
                </div>
                ...
                <div class="px-4 py-5 bg-gray-50 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $application->student->email }}</dd>
                </div>
                <div class="px-4 py-5 bg-white sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                ...
                <div class="px-4 py-5 bg-gray-50 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    <form method="POST" action="/application/accept/{{$application->id}}">
                        ...
                        @method('PATCH')
                        <!-- status input -->
                        <label class="inline-flex items-baseline p-4 mb-6 border-2 border-black rounded input-field">
                                <span class="flex-none leading-none text-black select-none text-dusty-blue-darker ">Accept</span>
                                <div class="flex-1 leading-none">
                                    <input  type="checkbox" class="w-full pl-1 ml-2 bg-transparent focus:outline-none" name="status" />
                                    @error('stauts')
                        ...
                        <x-form.textarea name="feedback"/>
                        <button class="px-4 py-2 mt-8 font-bold text-white bg-blue-500 rounded-full hover:bg-blue-700" type="submit">
                            Submit
                        </button>
